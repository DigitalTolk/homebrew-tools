...
FORMULA_PATH = Pathname.new(ENV.fetch("KEEL_FORMULA_PATH", ROOT/"Formula/keel.rb"))
RELEASE_URL = URI("https://api.github.com/repos/DigitalTolk/keel/releases/latest")
ASSETS = {
  "darwin_arm64" => "keel_Darwin_arm64.tar.gz",
  "darwin_x86_64" => "keel_Darwin_x86_64.tar.gz",
  "linux_arm64" => "keel_Linux_arm64.tar.gz",
  "linux_x86_64" => "keel_Linux_x86_64.tar.gz",
}.freeze
...
updated = replace_once(
  updated,
  %r{(keel_Darwin_arm64\.tar\.gz"\n\s+sha256 ")[a-f0-9]{64}(")},
  "\\1#{digests.fetch("darwin_arm64")}\\2",
  "Darwin arm64 sha256",
)
updated = replace_once(
  updated,
  %r{(keel_Darwin_x86_64\.tar\.gz"\n\s+sha256 ")[a-f0-9]{64}(")},
  "\\1#{digests.fetch("darwin_x86_64")}\\2",
  "Darwin x86_64 sha256",
)
updated = replace_once(
  updated,
  %r{(keel_Linux_arm64\.tar\.gz"\n\s+sha256 ")[a-f0-9]{64}(")},
  "\\1#{digests.fetch("linux_arm64")}\\2",
  "Linux arm64 sha256",
)
updated = replace_once(
  updated,
  %r{(keel_Linux_x86_64\.tar\.gz"\n\s+sha256 ")[a-f0-9]{64}(")},
  "\\1#{digests.fetch("linux_x86_64")}\\2",
  "Linux x86_64 sha256",
)

if updated == content
  puts "keel formula already at #{version}"
else
  FORMULA_PATH.write(updated)
  puts "updated keel formula to #{version}"
end
