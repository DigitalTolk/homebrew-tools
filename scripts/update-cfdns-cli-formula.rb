...
FORMULA_PATH = Pathname.new(ENV.fetch("CFDNS_CLI_FORMULA_PATH", ROOT/"Formula/cfdns-cli.rb"))
RELEASE_URL = URI("https://api.github.com/repos/DigitalTolk/cfdns-cli/releases/latest")
ASSETS = {
  "darwin_arm64" => "cfdns-cli_Darwin_arm64.tar.gz",
  "darwin_x86_64" => "cfdns-cli_Darwin_x86_64.tar.gz",
  "linux_arm64" => "cfdns-cli_Linux_arm64.tar.gz",
  "linux_x86_64" => "cfdns-cli_Linux_x86_64.tar.gz",
}.freeze
...
puts "latest cfdns-cli release: #{tag}"
...
updated = replace_once(
  updated,
  %r{(cfdns-cli_Darwin_arm64\.tar\.gz"\n\s+sha256 ")[a-f0-9]{64}(")},
  "\\1#{digests.fetch("darwin_arm64")}\\2",
  "Darwin arm64 sha256",
)
updated = replace_once(
  updated,
  %r{(cfdns-cli_Darwin_x86_64\.tar\.gz"\n\s+sha256 ")[a-f0-9]{64}(")},
  "\\1#{digests.fetch("darwin_x86_64")}\\2",
  "Darwin x86_64 sha256",
)
updated = replace_once(
  updated,
  %r{(cfdns-cli_Linux_arm64\.tar\.gz"\n\s+sha256 ")[a-f0-9]{64}(")},
  "\\1#{digests.fetch("linux_arm64")}\\2",
  "Linux arm64 sha256",
)
updated = replace_once(
  updated,
  %r{(cfdns-cli_Linux_x86_64\.tar\.gz"\n\s+sha256 ")[a-f0-9]{64}(")},
  "\\1#{digests.fetch("linux_x86_64")}\\2",
  "Linux x86_64 sha256",
)

if updated == content
  puts "cfdns-cli formula already at #{version}"
else
  FORMULA_PATH.write(updated)
  puts "updated cfdns-cli formula to #{version}"
end
